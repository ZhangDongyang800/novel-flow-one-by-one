...
1. **场景规划**：
   ...
   - 确定本章的"反差锚点"——一个让本章区别于其他章的独特元素，在写每个场景时有意识地体现它。好的反差锚点示例：本章所有对话都是试探而非直白、主角在本章第一次表现出某种隐藏性格、本章的高潮发生在正午而非夜晚、本章唯一的光源是月光所以所有描写都偏冷色调

2. **逐场景写入**（正文写入 `【书名】/第X卷/chapter-xxx.md`）：

   **提醒：Scene/Sequel 交替是引导，不是铁律。** guide.md 中的 Scene/Sequel 交替是基础框架，但合法的打破能避免公式化。以下情况允许打破：
   - 两个行动场景连续推进时，中间用极短的反应瞬间（一句话或一个动作）代替完整 Sequel：("他愣了一下。然后继续。")
   - 反应场景可以压缩到 1-2 句，不一定是完整段落：("他没说话，把烟掐了。")
   - 如果本章反差锚点要求特定节奏（如"沉默中的爆发"），可以 2 个行动场景连在一起制造压抑，但第 3 个必须是反应场景
   - **判断标准**：打破交替后读起来自然吗？读者会不会感到疲劳（连续行动）或无聊（连续反应）？如果会 → 恢复交替
   ...
   c. 写完场景后停下来，回答以下 5 个问题：
      1. **读者情绪**：读者读完这个场景后的情绪是什么？如果和上一个场景相同，下一个场景必须转换情绪
      2. **节奏推进**：下一个场景应该是行动场景还是反应场景？是否需要打破常规的 Scene/Sequel 交替（见下方"提醒"）？
      3. **钩子检查**：这个场景结束时读者是否想知道"然后呢"？如果没有，补一个钩子——一个动作、一句话、一个悬念
      4. **角色代入**：如果我是[当前场景的主视角角色]，我真的会这么做/这么说吗？对照人物卡中的驱动力、恐惧和关键特征——角色的行为必须与其核心特质一致，不能每个角色说话做事都一个样
      5. **角色特质推进**：这个场景是否推进或揭示了该角色的至少一项核心特质（驱动力或恐惧）？如果都没有，这个场景在"人物感"上可能不及格
